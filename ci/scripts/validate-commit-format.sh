...
while IFS= read -r hash; do
  ...
  if echo "$FULL_MSG" | grep -qiE '(generated by|coded with|created with|authored by (claude|cursor|codex|copilot)|claude code|cursor ai|codex agent|copilot)'; then
    echo "FORGE: AI attribution is not allowed in ${hash:0:8}: $SUBJECT"
    COMMIT_FAILED=1
  fi
